...
beforeEach(async () => {
  ...
  const addedUser = helper.initialBlogs.map((blog) => ({
    ...blog,
    user: gUser._id,
  }))
...
describe('adding new blogs', () => {
  ...
  test('blogs that are added without title or url will get status code 400 as response', async () => {
    ...
    await api.post('/api/blogs').send(withoutTitle).set(headers).expect(400)

    await api.post('/api/blogs').send(withoutUrl).set(headers).expect(400)
